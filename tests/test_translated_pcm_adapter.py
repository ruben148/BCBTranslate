...
def test_partial_chunk_then_rest() -> None:
    ...
    # First pop consumed 100 + pad; second should take 320 from 500 remaining logic
...
def test_overflow_drops_oldest() -> None:
    a = TranslatedPcmStereoAdapter(
        16000, frame_ms=20.0, max_buffer_seconds=0.05,
    )
    # Fill more than max_buffer_seconds worth of mono samples
    big = np.ones(2000, dtype=np.int16).tobytes()
    for _ in range(20):
        a.push_mono_pcm(big)
    # Should not explode; eventually we get finite frames
    for _ in range(30):
        a.pop_stereo_s16le_frame()
